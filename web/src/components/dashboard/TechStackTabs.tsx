      ...
      {stacks.map((stack) => (
        <button
          ...
          className={`
            ...
            ${selectedStack === stack ? 'text-sky-400' : 'text-slate-400 hover:text-slate-200'}
          `}
        >
          {stack}
          {selectedStack === stack && (
            <div className="absolute bottom-0 left-0 w-full h-0.5 bg-sky-400 shadow-[0_0_10px_rgba(56,189,248,0.5)]" />
          )}
        </button>
      ))}
    </div>
