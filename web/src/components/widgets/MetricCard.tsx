...
export const MetricCard: React.FC<MetricCardProps> = ({ title, value, subValue, loading }) => {
  return (
    <Card className="border-slate-700 bg-slate-800/50 backdrop-blur shadow-xl">
      <CardHeader>
        <CardTitle className="text-slate-200 text-sm font-medium uppercase tracking-wide">
          {title}
        </CardTitle>
      </CardHeader>
      <CardContent>
        {loading ? (
          <div className="animate-pulse h-16 bg-slate-700/20 rounded"></div>
        ) : (
          <div>
            <div className="text-5xl font-bold text-slate-100">{value}</div>
            {subValue && (
