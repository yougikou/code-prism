...
pub async fn upsert_template(
    ...
    core_config
        .project_templates
        .insert(name.clone(), template_config);
    ...
    *state.core_config.write().unwrap() = core_config;
    ...
    Json(serde_json::json!({"status": "ok", "message": format!("Template '{}' saved", name)}))
        .into_response()
}
...
pub async fn delete_template(
    ...
    *state.core_config.write().unwrap() = core_config;
    ...
    Json(serde_json::json!({"status": "ok", "message": format!("Template '{}' deleted", name)}))
        .into_response()
}
