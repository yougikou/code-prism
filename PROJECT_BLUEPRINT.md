...
**现状问题**：
- `/api/v1/scan` 目前只返回“started”，缺乏真实任务状态、进度、失败原因和产出的 `scan_id`。
- 后台扫描通过异步任务执行，但前端无法可靠跟踪完成、失败或对应结果。

**目标原则**：
- 扫描必须是可追踪、可失败、可恢复、可关联结果的后台任务，而不是一次无状态触发。

**负债改进要求**：
- 引入 `scan_jobs` 概念，明确 `queued / running / failed / completed` 状态。
- `POST /api/v1/scan` 返回 `job_id`，并提供 `GET /api/v1/scan-jobs/:id` 查询接口。
- 任务结果中必须包含 `progress`、`error_message`、最终 `scan_id`。
- `ExecutePage` 必须基于 job 状态形成完整闭环，而不是依赖刷新 run 列表“碰运气”看到结果。

#### B. 扫描错误传播与数据一致性

**现状问题**：
- Git walker / analyzer / 写库链路中的局部错误目前可能只写日志，不会稳定上抛到扫描结果层。
- 这会造成“部分失败但上层误判为成功”的风险。

**目标原则**：
- 扫描成功必须意味着结果完整可信；如果关键阶段失败，则系统必须显式记录为失败。

**负债改进要求**：
- `git_scanner` 中阻塞扫描任务的错误必须回传主流程，而不是仅 `eprintln!`。
- 扫描记录必须持久化状态与错误信息，禁止静默失败。
- 后续聚合与前端展示只允许消费 `completed` 状态的扫描结果。
- 为“部分失败”场景定义明确策略：要么失败退出，要么将部分结果标记为 degraded，不允许语义模糊。
